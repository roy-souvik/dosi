...
function main()
{
	...
	{
	$color_sql="SELECT * FROM media where title like '$_POST[txt_alpha]%' and category='latest'"  . " ORDER BY id asc LIMIT $GLOBALS[start],$GLOBALS[show]";
	$row=mysql_fetch_array(mysql_query("select count(*) FROM media where title like '$_POST[txt_alpha]%' and category='latest'"  . " ORDER BY id asc "));
	$count=$row[0];
	}
	if ($_POST[search_mode]=="SEARCH")
	{
	$color_sql="SELECT * FROM media where title like '".stripslashes($_POST[txt_search])."%' and category='latest'" . " ORDER BY title LIMIT $GLOBALS[start],$GLOBALS[show]";
	$row=mysql_fetch_array(mysql_query("select count(*) FROM media where title like '".stripslashes($_POST[txt_search])."%'and category='latest'" . " ORDER BY id asc"));
	$count=$row[0];
	}
	if ($_POST[search_mode]=="")
	{
	$color_sql="SELECT *  FROM media  ORDER BY id asc LIMIT $GLOBALS[start],$GLOBALS[show]";
	$row=mysql_fetch_array(mysql_query("select count(*) FROM media  ORDER BY id asc"));
	$count=$row[0];
	}
...
<table width="99%" align="center" border="0" cellpadding="5" cellspacing="1">
    ...
    <td align="right" width="6%"><a href="javascript:add_color();" title=" Add Media Details"><img src="images/plus_icon.gif" border="0"></a></td>
		</tr>
	</table>
...
<table width="99%" align="center" border="0" cellpadding="5" cellspacing="2"  class="ThinTable">
  ...
  <tr class="text_normal"> 
    ...
    <td width="14%" bgcolor="#CFC5BC"><strong> Date</strong></td>
	<td width="14%" bgcolor="#CFC5BC"><strong> Title</strong></td>
	<td width="14%" bgcolor="#CFC5BC"><strong> Links</strong></td>	
  ...
  <tr onMouseOver="this.bgColor='<?=SCROLL_COLOR;?>'" onMouseOut="this.bgColor=''" class="text_small"> 
    ...
    <td align="center"><a href="javascript:active_deactive(<?=$rec['id'];?>,<?=$GLOBALS[start]?>)" title="Activate Deactivate Status"> 
      <?
			...
			{				
	 ?>
      			<img src="images/icon_active.png"  border="0" title="Click to Deactivate Media"> 
      <? 	} 
	  		else
			{ ?>
      			<img src="images/icon_inactive.png"  border="0" title="Click to Activate Media">
      <? 
	  		} 
	  ?>
      </a></td>
    <td align="center"><a href="javascript:Edit(<?=$rec['id'];?>,<?=$GLOBALS[start]?>);" title="Edit Media Details"><img src="images/edit_icon.gif" border="0"></a></td>
    <td align="center"><a href="javascript:Delete(<?=$rec['id'];?>,<?=$GLOBALS[start]?>)" title="Delete Media Details"><img name="xx" src="images/delete_icon.gif" border="0"></a></td> 
  </tr>
...
{ 
	...
	{	
		...
		$media_date=$rec['media_date'];
		$new_media_date=explode("-",$media_date);
		$day=$new_media_date[2];
		$month=$new_media_date[1];
		$year=$new_media_date[0];
		$title=$rec['title'];
		$links=$rec['links'];
	}
	else{
...
{				
	...
	var textarea = tinyMCE.get('links').getContent(); 
...
<form action="<?=$_SERVER['PHP_SELF']?>" method="post" enctype="multipart/form-data" name="frmedit" onSubmit="return check();" >
  ...
  <tr>
    <td align="center"><table width="100%" align="center"  cellpadding="4" cellspacing="4" >
          ...
          <?php }?>
          <tr> 
            <td width="16%" class="text_small" align="left">Title</td>
            <td width="2%" class="text_small" >:</td>
            <td width="82%" align="left">
				<input type="text" id="title" name="title" size="35" value="<?=$rec['title']?>"> 
            </td>
          </tr>
          
          <tr> 
            <td class="text_small" align="left">Links</td>
            <td class="text_small">:</td>
            <td align="left">
    
				<textarea id="links" name="links" rows="10" cols="50" style="width: 70%">
                       	<?=stripslashes($links)?>
				</textarea>
          ...
          <tr> 
            ...
            <td class="point_txt"><input name="submit" type="submit" class="inplogin" value="<?=$_REQUEST['mode']=='add'?'Add':'Update'?>"> 
              &nbsp; 
                 <input name="button" type="button" class="inplogin" onClick="javascript:window.location='admin_media.php';"  value="Cancel"> 
            </td>
          </tr>
        </table></td>
  </tr>
...
{
	$sql_query="DELETE FROM media WHERE id='".$id."'";
 ...
 {  
  	if($row_id=='') {
		$links=preg_replace(array('#<p>#', '#</p>#'), '', $_POST['links'], 1);
...
function active_deactive($id)
{
	...
	$sql = "UPDATE media SET is_active = if(is_active = 'N','Y','N') WHERE id = '$id'";
	mysql_query($sql);
